...
struct ContactDetailView_Previews : PreviewProvider {
    static var previews: some View {
        ...
        return ContactDetailView(contact: Contact(id: UUID().uuidString, firstname: "longFirstNameTest", lastname: "longLastNameTest", email: "longEmailTest@web.de", phone: "123456789", dob: "12.12.2001", note: "", company_uids: [], tag_uids: [], project_uids: []))
    ...
    var body: some View {
        VStack(alignment: .leading) {
            HStack {
                ...
                Button(action: {
                    NSWorkspace.shared.open(URL(string: "tel://\(selectedContact.phone)")!)
                }, label: {
                    HStack {
                        Image(systemName: "phone").foregroundColor(Color.chimpPrimary)
                        Text(selectedContact.phone).foregroundColor(Color.chimpLightText).padding(.leading, -5)
                    }
                })
            }
            HStack {
                Button(action: {
                    ...
                    HStack {
                        ...
                        Text(selectedContact.dob).foregroundColor(Color.chimpLightText).padding(.leading, -5)
                    }
                })
                Button(action: {
